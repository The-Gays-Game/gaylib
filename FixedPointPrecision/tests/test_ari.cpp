...
import helpers;
      ...
      for (const TestType dividend:divisorSamples) {
        for (const TestType divisor:divisorSamples) {
          ...
          if (rt>=NL<TestType>::min() && rt<=NL<TestType>::max()) {
            ...
            //CAPTURE(Tt(qt),Tt(qy),Tt(dividend),Tt(divisor));
  ...
  TEMPLATE_TEST_CASE("promotion", "", int, unsigned int) {
    ...
    SECTION("sRemQuo") {
        for (const TestType dividend:samples) {
          for (const TestType divisor:samples) {
            CAPTURE(Tt(dividend),Tt(divisor));
            int qt;
            double rt=std::remquo(dividend,divisor,&qt);
            if (rt>=NL<TestType>::min() && rt<=NL<TestType>::max()) {
              auto[qy,ry]=sRemQuo(dividend,divisor);
